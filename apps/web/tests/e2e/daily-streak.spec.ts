...
import { addDays, localDate } from "@veylo/backend/domain/planner";
...
test("streak API counts completed rounds once and rejects incomplete or fabricated duration", async ({
  ...
  expect((await request.get("/api/v1/streak")).status()).toBe(401);
  expect(await (await page.request.get("/api/v1/streak")).json()).toMatchObject(
    {
      current: 0,
      todayComplete: false,
    },
  ...
  const incomplete = await (
    await page.request.post("/api/v1/arcade-rounds", {
      data: { game: "runner", duration: 30 },
    })
  ).json();
  const short = await page.request.post(
    `/api/v1/arcade-rounds/${incomplete.id}`,
    {
  ...
  const invalid = await page.request.post(`/api/v1/arcade-rounds/${round.id}`, {
    data: { elapsed: 30, speechSeconds: 0, answers: 1 },
  });
  expect(invalid.status()).toBe(400);
  const results = await Promise.all(
    Array.from({ length: 2 }, () =>
      page.request.post(`/api/v1/arcade-rounds/${round.id}`, {
  ...
  expect(
    (
      await page.request.post(`/api/v1/arcade-rounds/${randomUUID()}`, {
        data: { elapsed: 0, speechSeconds: 0, answers: 8 },
      })
    ).status(),
  ).toBe(404);
