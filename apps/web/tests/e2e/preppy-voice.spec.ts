...
test("Vey completes hands-free and hold-to-talk turns, retries TTS, and saves subtitles", async ({
  ...
  await page.addInitScript(() => {
    ...
    let mouthOpening = 0;
    Object.defineProperty(window, "veyMouthOpening", {
      get: () => mouthOpening,
    });
    new MutationObserver((records) => {
      for (const record of records) {
        ...
        if (
          element.matches?.("[data-vey-mouth]") &&
          element.closest(".preppy-stage")
        )
          mouthOpening = Math.max(
            mouthOpening,
            Number(element.getAttribute("ry")),
          );
      }
    }).observe(document, {
      subtree: true,
  ...
  await page.route("**/api/v1/chat/audio/transcribe?*", async (route) => {
  ...
  await page.route("**/api/v1/chat/audio/speak", async (route) => {
    if (ttsFailure) {
      ttsFailure = false;
      await route.fulfill({
        status: 502,
  ...
  await expect
    .poll(
      () =>
        page.evaluate(
          () =>
            (window as unknown as { veyMouthOpening: number }).veyMouthOpening,
        ),
      { timeout: 30000 },
    )
    .toBeGreaterThan(2);
...
test("Vey respects reduced motion and uses the animated Vey vector character", async ({
  ...
  await expect(
    page.locator(
      ".preppy-introduction [data-vey-outline], .preppy-introduction [data-vey-left], .preppy-introduction [data-vey-right]",
    ),
  ).toHaveCount(3);
  await expect(
...
test("voice API requires authentication and enforces thread, message and WAV boundaries", async ({
  ...
  for (const path of [
    ...
    "/api/v1/chat/audio/welcome",
    `/api/v1/chat/audio/transcribe?thread=${randomUUID()}`,
  ]) {
  ...
  try {
    ...
    expect(
      (
        await page.request.post(
          `/api/v1/chat/audio/transcribe?thread=${otherThreadId}`,
    ...
    expect(
      (
        await page.request.post(
          `/api/v1/chat/audio/transcribe?thread=${threadId}`,
    ...
    expect(
      (
        await page.request.post("/api/v1/chat/audio/speak", {
...
test("floating Vey chat preserves text through network retry and plays a completed reply", async ({
  ...
  await page.route("**/api/v1/chat/audio/speak", (route) =>
    route.fulfill({ json: { url: "/preppy-chat.wav" } }),
  );
  await page.route("**/api/v1/chat/audio/welcome", (route) =>
    route.fulfill({ json: { url: "/preppy-chat.wav" } }),
  );
...
test("floating chat transcribes a voice message and releases the microphone on close", async ({
  ...
  let transcriptions = 0;
  await page.route("**/api/v1/chat/audio/transcribe?*", (route) => {
    ...
    transcriptions++;
    return route.fulfill({
      json: { text: "How can I practise my vocabulary?" },
    });
  });
  ...
  await expect(dialog.getByRole("log")).toContainText(
    "Try reviewing a few words each day.",
  );
  await expect(dialog.locator('[data-role="user"]')).toHaveText(
    "How can I practise my vocabulary?",
  );
  expect(transcriptions).toBe(1);
  await dialog.getByRole("button", { name: "Record a voice message" }).click();
  await expect(dialog.getByRole("textbox")).toHaveAttribute(
...
test("microphone rejection is recoverable and changing personality starts a separate session", async ({
  ...
  await page
    ...
    .click();
  await page.getByRole("button", { name: "Back to Vey", exact: true }).click();
  await expect(
