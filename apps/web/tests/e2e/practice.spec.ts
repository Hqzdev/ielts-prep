...
test("Reading shows an estimated band, evidence and consistent progress", async ({
  ...
  await expect(
    page.getByText("Correct answer", { exact: true }).filter({ visible: true }),
  ).toBeVisible();
  await page
    .locator(".feedback-row")
    .filter({ hasText: "a check after every return", visible: true })
    .getByText("Evidence · paragraph C", { exact: true })
    .click();
  await expect(
    page
      .getByText("a check after every return", { exact: true })
      .filter({ visible: true }),
  ).toBeVisible();
  await page.screenshot({
