import { api, apiData, ApiError } from "./api";
import { DraftStore, type LocalDraft } from "./draft-store";
import type { Answer, Attempt } from "@veylo/backend/domain/attempt";
import { AppError, errorMessage } from "@veylo/backend/domain/errors";
...
export class DraftSession {
  ...
  async flush(action?: "pause" | "resume"): Promise<void> {
    ...
    this.pending = apiData(
      api.PATCH("/attempts/{id}", {
        params: { path: { id: this.state.attempt.id } },
        body: { answer, revision, action },
      }),
    )
      .then(async (attempt) => {
        this.dirty =
