...
import type {
  ...
  VocabularyQuiz,
  QuizAnswer,
} from "@veylo/backend/domain/vocabulary";
...
export function Vocabulary({
  ...
  async function quiz() {
    ...
    setError(null);
    try {
      const quiz = await apiData(
        api.POST("/vocabulary/quizzes", {
          body: { topic: topic || undefined, personal },
        }),
      );
  ...
  async function save(word: VocabularyWord) {
    try {
      await apiData(
        api.POST("/vocabulary/saved", {
          body: { wordId: word.id, saved: !word.saved },
        }),
      );
      setWords(
        ...
        <div>
          ...
          <div>
            ...
            <WordComposer
              ...
              onSaved={async () =>
                setWords(await apiData(api.GET("/vocabulary")))
              }
            />
          </div>
        </div>
...
export function VocabularyTest({ initial }: { initial: VocabularyQuiz }) {
  ...
  async function finish() {
    ...
    try {
      setResult(
        await apiData(
          api.POST("/vocabulary/quizzes/{id}/submit", {
