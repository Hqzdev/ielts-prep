...
} from "@veylo/backend/domain/conversation-feedback";
import type { ConversationMessage } from "@veylo/backend/domain/voice-conversation";
...
export function ConversationSummary({
  ...
  useEffect(() => {
    ...
    apiData(
      api.POST("/chat/feedback", { body: { threadId }, signal: abort.signal }),
    )
      .then((result) => {
        if (!abort.signal.aborted) {
          if (result.status === "ready") setFeedback(result.feedback);
