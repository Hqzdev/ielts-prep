"use client";
import type { LearningService } from "@veylo/backend/application/services/learning";
...
import type { StudySession } from "@veylo/backend/application/ports/learning";
import type { Attempt } from "@veylo/backend/domain/attempt";
import type { Profile } from "@veylo/backend/domain/profile";
import { currentOverallBand } from "@veylo/backend/domain/statistics";
...
function SessionRow({ session }: { session: StudySession }) {
  ...
  async function start() {
    ...
    setError(null);
    try {
      const attempt = await apiData(
        api.POST("/learning/{id}", {
