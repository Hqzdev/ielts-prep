...
import { wordCount } from "@veylo/backend/domain/attempt";
import { categoryLabels } from "@veylo/ui-web/domain/formatting";
import { type Assessment } from "@veylo/backend/domain/assessment";
import { taskPartLabel } from "@veylo/ui-web/domain/formatting";
  ...
  async function play() {
    try {
      const response = await apiData(
        api.GET("/audio/{id}", { params: { path: { id: id } } }),
      );
      setUrl(response.url);
      setError(null);
    } catch (error) {
...
export function Result({
  ...
  async function revise() {
    setBusy(true);
    try {
      const revision = await apiData(
        api.POST("/attempts/{id}/revisions", {
          params: { path: { id: attempt.id } },
          body: {},
        }),
      );
  ...
  async function retry() {
    ...
    try {
      setAssessment(
        await apiData(
          api.POST("/attempts/{id}/retry-assessment", {
            params: { path: { id: attempt.id } },
            body: {},
          }),
        ),
