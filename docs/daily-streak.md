...
Unit cases: tests/daily-streak.test.ts. Transactional database checks: infra/supabase/tests/daily_streak.sql. Browser and API checks: tests/e2e/daily-streak.spec.ts and tests/e2e/arcade-round.spec.ts. The SQL checks run inside a transaction and roll back all fixtures.
