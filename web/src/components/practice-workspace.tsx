...
import { taskPartLabel, formatLabels, topicLabels } from "@/domain/task";
...
export function PracticeWorkspace({
  ...
  async function submit(expired = false) {
    ...
    try {
      ...
      await api(`/api/attempts/${attempt.id}/submit`, {
        method: "POST",
        body: "{}",
      });
      ...
      if (error instanceof AppError && error.code === "DEADLINE_EXPIRED") {
        await submit(true);
        return;
      }
