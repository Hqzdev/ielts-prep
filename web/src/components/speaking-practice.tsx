...
import { formatDuration, type Attempt, type Answer } from "@/domain/attempt";
...
export function SpeakingPractice({
  ...
  async function playQuestion(questionIndex = index) {
    ...
    try {
      const { url } = await api<{ url: string }>("/api/audio/question", {
        method: "POST",
        body: JSON.stringify({ taskId: task.id, questionIndex }),
      });
      ...
      {phase === "setup" && (
        <>
          ...
          <p className="small muted">
            Allow microphone access. You will hear the examiner&apos;s questions and
            answer aloud.
          </p>
