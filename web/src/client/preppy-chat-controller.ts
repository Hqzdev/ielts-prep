...
import { isVeyExpression } from "@/domain/vey-expressions";
import type { PreppyExpression } from "@/domain/preppy";
...
export class PreppyChatController {
  ...
  async send(content: string) {
    ...
    await this.run(() =>
      this.stream("/api/chat/messages", {
        threadId: this.snapshot.threadId,
        content: message.content,
      }),
    );
  }
  private async stream(path: string, body: object) {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        result.error?.message ?? "Vey is unavailable. Please retry.",
      );
    }
  ...
  private async recover(assistantId: string) {
    const messages = await api<ConversationMessage[]>(
      `/api/chat/messages?thread=${this.snapshot.threadId}`,
      { signal: this.abort.signal },
    );
    ...
    await this.stream("/api/chat/messages", {
      threadId: this.snapshot.threadId,
  ...
  async record() {
    ...
    await this.run(async () => {
      ...
      if (!this.snapshot.threadId) await this.stream("/api/chat/session", {});
  ...
  async finishRecording() {
    ...
    this.retryOperation = async () => {
      const { text } = await api<{ text: string }>(
        `/api/chat/audio/transcribe?thread=${this.snapshot.threadId}`,
        {
          method: "POST",
      ...
      const { url } = await api<{ url: string }>(
          ...
          : "/api/chat/audio/speak",
        {
          ...
          body: JSON.stringify(
            messageId === "welcome"
              ? {}
              : { threadId: this.snapshot.threadId, messageId },
          ),
          signal: this.abort.signal,
        },
      );
