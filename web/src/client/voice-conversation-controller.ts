...
export class VoiceConversationController {
  ...
  async start(preferences = this.snapshot.preferences) {
    ...
    try {
      ...
      await this.run(() => this.streamReply("/api/chat/session", {}));
    } catch (error) {
  ...
  private async sendText() {
    ...
    await this.streamReply("/api/chat/messages", {
  ...
  private async streamReply(
    ...
    const response = await fetch(path, {
      ...
      body: JSON.stringify({ ...body, ...this.snapshot.preferences }),
      signal: this.abort.signal,
    });
    if (!response.ok) {
  ...
  private async speakReply() {
    ...
    this.audio.mute(true);
    const { url } = await api<{ url: string }>("/api/chat/audio/speak", {
      method: "POST",
      body: JSON.stringify({
        threadId: this.snapshot.threadId,
