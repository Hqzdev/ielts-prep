...
import {
  VoiceChatService,
  type VoiceChatProvider,
} from "@/server/services/voice-chat";
import { TutorService } from "@/server/services/tutor";
import { WavCodec } from "@/domain/wav";
import { AppError } from "@/domain/errors";
import { preppyWelcome } from "@/domain/preppy";
...
vi.mock("@/server/services/learning", () => ({
...
vi.mock("@/server/repositories/practice", () => ({
...
beforeEach(() => {
  ...
  service = new VoiceChatService(database.client, provider);
});
...
describe("voice audio authorization and validation", () => {
  ...
  it("rejects foreign threads before reading audio or calling Gemini", async () => {
    await expect(
      service.transcribe("other", "thread", audioRequest()),
    ).rejects.toMatchObject({ status: 404 });
    expect(provider.transcribe).not.toHaveBeenCalled();
  });
  it("transcribes mono 16 kHz audio entirely in memory", async () => {
    expect(await service.transcribe("owner", "thread", audioRequest())).toEqual(
      { text: "I live in London." },
    );
  ...
  it.each([
    ...
    await expect(
      service.transcribe("owner", "thread", audioRequest(buffer)),
  ...
  it("reports Gemini failures without writing user audio", async () => {
    ...
    await expect(
      service.transcribe("owner", "thread", audioRequest()),
...
describe("session stream", () => {
  ...
  it("creates a thread and greeting without a fictional learner message", async () => {
    const result = await service.start(profile);
  ...
  it("enforces the existing daily message limit", async () => {
    ...
    await expect(service.start(profile)).rejects.toMatchObject({ status: 429 });
  ...
  it("persists partial text and retries an interrupted answer without adding learner messages", async () => {
    ...
    const response = await service.start(profile);
    ...
    await (
      await new TutorService(database.client, provider).respond(profile, {
        threadId: "new-thread",
        retryAssistantId: id,
      })
    ).text();
...
describe("conversation feedback boundaries", () => {
  ...
  it("checks ownership and short conversations before calling the provider", async () => {
    const { ConversationFeedbackService } =
      await import("@/server/services/conversation-feedback");
