...
test("renders the band, exact quotations and corrections from a stored assessment", async ({
  ...
  const created = await page.request.post("/api/attempts", {
    headers,
    data: { taskId: "w2-001", mode: "practice" },
  });
  const attempt = await created.json();
  await page.request.patch(`/api/attempts/${attempt.id}`, {
    headers,
    data: {
  ...
  await page.request.post(`/api/attempts/${attempt.id}/submit`, {
    headers,
    data: {},
  });
  const grade = {
