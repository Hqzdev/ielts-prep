...
class TaskHistory:
    def __init__(self, tasks: list[dict], task_type: str):
        self._texts = [
            self._task_text(task) for task in tasks if task.get("_task_type") == task_type
        ]
    ...
    def validate_novelty(self, task: dict) -> None:
        ...
        for text in self._texts:
            ...
            if candidate and previous and SequenceMatcher(None, candidate, previous).ratio() >= 0.92:
                raise ValueError("Task repeats or closely paraphrases a saved task; choose a different angle")
    ...
    def _task_text(task: dict) -> str:
        for key in ("prompt", "cue_card", "title"):
            if task.get(key):
                return task[key]
        return " ".join(question for question in task.get("questions", []) if isinstance(question, str))
        ...
        for attempt in range(3):
            response = self._client.models.generate_content(
                ...
                config={
                    ...
                    "max_output_tokens": min(specification.max_output_tokens * (attempt + 1), 32768),
                },
            )
            try:
                ...
                if attempt == 2:
                    raise ValueError(f"Задание не прошло проверку после 3 попыток: {error}") from error
                correction = "\nThe previous attempt failed validation. Regenerate a complete task and fix: " + str(error)[:1500]
...
def parse_json_response(response) -> dict:
    ...
    raw = "".join(
        part.text for part in candidate.content.parts or []
...
def save_tasks(tasks: list[dict]) -> None:
    with NamedTemporaryFile(mode="w", encoding="utf-8", dir=OUTPUT_FILE.parent, suffix=".json", delete=False) as temporary:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Генератор самостоятельных IELTS-style заданий")
    parser.add_argument("--count", type=int, default=10, help="Сколько заданий сгенерировать")
    parser.add_argument("--task", choices=TaskCatalog.task_names, default="writing_task2", help="Тип задания")
    parser.add_argument("--shuffle-start", action="store_true", help="Начать цикл тем и форматов со случайной позиции")
    ...
    with genai.Client(api_key=api_key) as client:
        generator = TaskGenerator(client, TaskCatalog(), os.environ.get("GEMINI_MODEL", MODEL))
        ...
        try:
            for index in range(args.count):
                ...
                save_tasks(existing)
                generated += 1
                label = task.get("topic_category") or task.get("topic") or task.get("chart_type") or task.get("title") or task.get("related_topic")
