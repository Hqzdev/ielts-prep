...
ESSAY_TOPICS = {
    ...
    "technology": (
        "automation of everyday services",
...
PART2_TOPICS = {
    ...
    "place": (
        "a quiet place you like",
        "a public place you have visited",
        "a place in your area you would recommend",
    ),
    "object": (
        "an object you use regularly",
        "something you have kept for a long time",
        "a gift that was useful to you",
    ),
    "event": (
        "a celebration you enjoyed",
        "a local event you attended",
        "an occasion when people worked together",
    ),
    "experience": (
        "a time you solved a practical problem",
        "a time you learned something outside school",
        "a time someone gave you helpful advice",
    ),
    "activity": (
        "an activity you would like to learn",
        "something you enjoy doing outdoors",
        "an activity you do with other people",
    ),
    "media": (
        "a story you found interesting",
        "a programme that taught you something",
        "a photograph that matters to you",
    ),
}

READING_TOPICS = (
    "how seed banks preserve plant diversity",
    ...
    "how people learn unfamiliar skills",
    "the conservation of historic buildings",
    "the role of wetlands",
    "how maps influence decisions",
)
...
class TaskSpec:
    ...
    def validate(self, data: dict) -> dict:
        ...
        if self.required_ending and not result["prompt"].endswith(self.required_ending):
            raise ValueError(
                f"Use the required task instruction: {self.required_ending}"
            )
        if result.get("task_type") == "two_part" and result["prompt"].count("?") != 2:
            raise ValueError(
                "A two-part task must contain exactly two direct questions"
            )
...
class TaskCatalog:
    ...
    def _speaking2(self, index: int, existing: list[dict]) -> TaskSpec:
        ...
        return TaskSpec(
            ...
            metadata={
                ...
                "speaking_seconds": {"minimum": 60, "maximum": 120},
            },
        )
    ...
    def _speaking3(self, index: int, existing: list[dict]) -> TaskSpec:
        cards = [
            ...
            if item.get("_task_type") == "speaking_part2" and item.get("cue_card")
        ]
        if cards:
        ...
        return TaskSpec(
            ...
            metadata={
                "part2_anchor": anchor,
                "time_minutes": {"minimum": 4, "maximum": 5},
            },
        )
