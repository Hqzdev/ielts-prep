...
def validate_english_text(value: str) -> str:
    ...
    if not value or re.search(r"[А-Яа-яЁё]", value) or "```" in value:
        raise ValueError(
            "Candidate-facing text must be non-empty English without Markdown fences"
        )
    return value
...
class WritingTask2(BaseModel):
    ...
    def validate_prompt_length(self):
        if not 25 <= len(self.prompt.split()) <= 100:
            raise ValueError(
                "Task 2 prompt must contain 25–100 words, excluding test instructions"
            )
        return self
    ...
    def validate_data(self):
        ...
        if self.chart_type == "pie_chart":
            if self.unit != "%" or len(self.data_series) < 3 or len(self.periods) > 2:
                raise ValueError(
                    "Pie charts need %, at least three categories and one or two pies"
                )
            for period in self.periods:
                if not math.isclose(
                    sum(series.values[period] for series in self.data_series),
                    100,
                    abs_tol=0.01,
                ):
...
class ReadingPassage(BaseModel):
    ...
    def validate_evidence(self):
        ...
        if not 650 <= word_count <= 950:
            raise ValueError(
                f"Reading passage must contain 650–950 words; received {word_count}"
            )
        ...
        for answer in self.answer_key:
            ...
            if " ".join(answer.evidence.split()) not in " ".join(paragraph.split()):
                raise ValueError(
                    f"Answer {answer.number}: evidence must be an exact passage quotation"
                )
        ...
        if any(
            verdicts.count(verdict) < 2 for verdict in ("TRUE", "FALSE", "NOT GIVEN")
        ):
            raise ValueError(
                "This practice set must contain at least two of each answer"
            )
        return self
